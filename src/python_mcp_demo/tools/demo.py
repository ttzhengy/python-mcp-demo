"""Demo 工具集 — 8 个基础 MCP 工具。

从原有的 server.py 迁移至 tools 层，保持功能完全一致。
"""
...
from python_mcp_demo.exceptions import MathExpressionError, MCPToolError
...
def _safe_eval(expression: str) -> float:
    ...
    def _eval(node: ast.AST) -> float:
        ...
        if isinstance(node, ast.Constant):
            ...
            raise MathExpressionError(f"不支持的常量类型: {type(node.value).__name__}")
        if isinstance(node, ast.UnaryOp):
            op_type = type(node.op)
            if op_type not in _SAFE_OPERATORS:
                raise MathExpressionError(f"不支持的一元运算符: {op_type.__name__}")
            return _SAFE_OPERATORS[op_type](_eval(node.operand))
        if isinstance(node, ast.BinOp):
            op_type = type(node.op)
            if op_type not in _SAFE_OPERATORS:
                raise MathExpressionError(f"不支持的二元运算符: {op_type.__name__}")
        ...
        if isinstance(node, ast.Name):
            ...
            if callable(value):
                raise MathExpressionError(f"{node.id} 是函数而非常量——请用 () 调用")
            return float(value)
...
def register_tools(server: FastMCP) -> None:
    ...
    async def fetch_url(url: str) -> dict[str, Any]:
        ...
        Returns:
            包含 ``status``、``content_preview`` 和 ``content_length`` 的字典。
        ...
        try:
            async with httpx.AsyncClient(
                ...
                text = response.text
                logger.debug("从 %s 获取了 %d 字节 (状态码=%d)", url, len(text), response.status_code)
                return {
            ...
            raise MCPToolError(f"HTTP {exc.response.status_code}: {exc.response.reason_phrase}") from exc
    ...
    async def calculate(expression: str) -> float:
        ...
        Raises:
            MathExpressionError: 表达式无效或包含不允许的语法。
        """
    ...
    async def random_number(min: float, max: float) -> float:
        ...
        """
        if min > max:
            raise MCPToolError(f"最小值 ({min}) 不能大于最大值 ({max})")
    ...
    async def current_time(timezone: str = "UTC") -> str:
        ...
        if timezone not in available_timezones():
            raise MCPToolError(
                f"未知时区: {timezone}。请使用有效的 IANA 时区，例如 'Asia/Shanghai'。"
            )
    ...
    async def echo(message: str, times: int = 1) -> list[str]:
        ...
        if times < 1 or times > 100:
            raise MCPToolError(f"重复次数 ({times}) 必须在 1 到 100 之间")
    ...
    async def count_words(text: str) -> dict[str, Any]:
        ...
        Returns:
            包含 ``char_count``、``word_count``、``line_count`` 和 ``top_words`` 的字典。
        """
        ...
        if not text.strip():
            return {"char_count": char_count, "word_count": 0, "line_count": line_count, "top_words": {}}
        ...
        return {"char_count": char_count, "word_count": word_count, "line_count": line_count, "top_words": top_words}
